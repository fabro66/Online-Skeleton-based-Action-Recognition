...
class NTUDataLoaders(object):
    def __init__(self, dataset='NTU', case=0, aug=1, seg=30):
        ...
        if self.dataset.startswith('NTU'):
            self.num_joints = 25
        elif self.dataset.startswith('GAST'):
            self.num_joints = 17
    ...
    def get_train_loader(self, batch_size, num_workers):
        if self.aug == 0:
            return DataLoader(self.train_set, batch_size=batch_size,
                              shuffle=True, num_workers=num_workers, collate_fn=self.collate_fn_fix_val,
                              pin_memory=False, drop_last=True, worker_init_fn=init_seed)
        elif self.aug == 1:
            return DataLoader(self.train_set, batch_size=batch_size,
                              shuffle=True, num_workers=num_workers, collate_fn=self.collate_fn_fix_train,
                              pin_memory=True, drop_last=True, worker_init_fn=init_seed)

    def get_val_loader(self, batch_size, num_workers):
            return DataLoader(self.val_set, batch_size=batch_size,
                              shuffle=False, num_workers=num_workers, collate_fn=self.collate_fn_fix_val,
                              pin_memory=True, drop_last=True, worker_init_fn=init_seed)

    def get_test_loader(self, batch_size, num_workers):
        return DataLoader(self.test_set, batch_size=batch_size,
                          shuffle=False, num_workers=num_workers, collate_fn=self.collate_fn_fix_test,
                          pin_memory=True, drop_last=True, worker_init_fn=init_seed)
    ...
    def create_datasets(self):
        if self.dataset == 'NTU60':
            if self.case == 0:
                ...
                self.metric = 'CV'
            path = osp.join('./data/ntu60/statistics/NTU60', 'NTU_' + self.metric + '.h5')
        elif self.dataset == 'NTU120':
            if self.case == 0:
                self.metric = 'CSub'
            elif self.case == 1:
                self.metric = 'CSet'
            path = osp.join('./data/ntu120/statistics/NTU120', 'NTU_' + self.metric + '.h5')
        elif self.dataset == 'GAST60':
            if self.case == 0:
                self.metric = 'CS'
            elif self.case == 1:
                self.metric = 'CV'
            path = osp.join('./data/gast60/statistics/GAST60', 'GAST_' + self.metric + '.h5')
        elif self.dataset == 'GAST120':
            if self.case == 0:
                self.metric = 'CSub'
            elif self.case == 1:
                self.metric = 'CSet'
            path = osp.join('./data/gast60/statistics/GAST120', 'GAST_' + self.metric + '.h5')
    ...
    def collate_fn_fix_train(self, batch):
        ...
        # theta 是弧度值
        if self.dataset in ['NTU60', 'GAST60']:
            if self.case == 0:
                theta = 0.3
            elif self.case == 1:
                theta = 0.5
        elif self.dataset in ['NTU120', 'GAST120']:
            theta = 0.3
    ...
    def Tolist_fix(self, joints, y, train=1):
        ...
        for idx, seq in enumerate(joints):
            ...
            for i in range(len(seq)):
                if (seq[i, :] == np.zeros((1, self.num_joints*3*2))).all():
                    zero_row.append(i)
    ...
    def turn_two_to_one(self, seq):
        new_seq = list()
        body_joints = self.num_joints * 3
        for idx, ske in enumerate(seq):
            if (ske[0:body_joints] == np.zeros((1, body_joints))).all():
                new_seq.append(ske[body_joints:])
            elif (ske[body_joints:] == np.zeros((1, body_joints))).all():
                new_seq.append(ske[0:body_joints])
            else:
                new_seq.append(ske[0:body_joints])
                new_seq.append(ske[body_joints:])
